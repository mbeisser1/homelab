...
def guess_direction_and_parties(msg, self_emails: List[str]) -> Tuple[str, Optional[str], Optional[str], Optional[str]]:
    ...
    gmailish = [c for c in candidates if c[1] and ('@gmail.' in c[1].lower() or c[1].lower() in se)]
...
def combine_for_person(person_key: str, messages: List[ParsedMessage], outdir: str):
    ...
    combined['To'] = f"{subj_name} <{addr or person_key}>" if addr or name else subj_name or 'unknown'
    ...
    for idx, pm in enumerate(messages, 1):
        ...
        combined.add_attachment(pm.raw_bytes,
                                maintype='message',
                                subtype='rfc822',
...
def main():
    ap = argparse.ArgumentParser(description="Combine SMS/MMS .eml files per person into combined .eml and JSON (attachments embedded).")
    ap.add_argument('inputs', nargs='+', help="Input .eml files or globs")
    ap.add_argument('-o', '--outdir', default=None, help="Output directory (default: ./combined_eml_out_YYYYmmdd_HHMMSS)")
