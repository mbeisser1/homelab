...
ALIASES = {
    ...
    "Robert McCroy Jr": {"names": ["robert mccroy jr","robert mcroy jr","mcroyr gmail com"], "numbers": ["3212462167"]},
...
def parse_date(date_hdr, x_val, backup_time, file_path):
    ...
    if date_hdr:
        try: return parsedate_to_datetime(date_hdr)
        except: pass
    if backup_time:
        try: return parsedate_to_datetime(backup_time)
        except: pass
    try:
    ...
    if msg.is_multipart():
        for part in msg.walk():
            ...
            if payload is None:
                ...
                if isinstance(raw, str):
                    payload = raw.encode(part.get_content_charset() or "utf-8", "replace")
                else:
                    payload = b""

            if ctype == "text/plain" and text_content is None:
                try:
                    text_content = payload.decode(part.get_content_charset() or "utf-8", "replace").strip()
                except Exception:
        ...
        if payload is None:
            ...
            if isinstance(raw, str):
                ...
                payload = b""
        try:
            text_content = payload.decode(msg.get_content_charset() or "utf-8", "replace").strip()
        except Exception:
            text_content = payload.decode("utf-8", "replace").strip()
...
def direction(from_hdr,to_hdr):
    ...
    if any(e in f for e in YOUR_EMAILS): return "out"
    if any(e in t for e in YOUR_EMAILS): return "in"
    return None
...
def main():
    if len(sys.argv)<2:
        print(f"Usage: {sys.argv[0]} INPUT_DIR [OUTPUT_DIR] [--phonebook=PATH] [--glob=PATTERN]")
        sys.exit(1)

    inp = Path(sys.argv[1])
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_root = Path(sys.argv[2]) if len(sys.argv)>=3 and not sys.argv[2].startswith("--") else Path(f"./eml_to_json_{timestamp}")

    patt="*.eml"
    phonebook_path=Path("./phonebook.json")
    for arg in sys.argv[2:]:
        if arg.startswith("--glob="): patt=arg.split("=",1)[1]
        if arg.startswith("--phonebook="): phonebook_path=Path(arg.split("=",1)[1])
    ...
    buckets={}
    global_summary={}
    parsed_count=0; written_count=0; dupe_count=0
    ...
    for i,p in enumerate(files,1):
        ...
        name_from_headers = extract_display_name(from_hdr if direction(from_hdr,to_hdr)!="out" else to_hdr)
        ...
        bucket=buckets.setdefault((canon,year),
            {"contact":{"canonical_id":canon,"display_name":canon,"numbers":[],"emails":[]},
             "meta":{"message_count":0,"first_timestamp":None,"last_timestamp":None},
             "messages":[], "seen":set()})
        ...
        rec={"timestamp":ts,"direction":dirn,"from":from_hdr,"to":to_hdr,"text":text,"attachments":imgs,"file":str(p)}
        key=(ts,dirn,text)
        ...
        written_count+=1
        if ts:
            if not bucket["meta"]["first_timestamp"] or ts<bucket["meta"]["first_timestamp"]:
                bucket["meta"]["first_timestamp"]=ts
            if not bucket["meta"]["last_timestamp"] or ts>bucket["meta"]["last_timestamp"]:
                bucket["meta"]["last_timestamp"]=ts

        if i%1000==0: print(f"...processed {i}/{len(files)} files")
    ...
    for num, counts in learned_names.items():
        ...
        if best_name:
            if num not in phonebook:
                ...
                if normalize_name_key(existing) == normalize_name_key(pretty_phone(num)):
                    phonebook[num] = best_name

    for (canon,year),blob in buckets.items():
        blob["messages"].sort(key=lambda m:m["timestamp"] or "")
        person_dir=out_root/safe_filename(canon); person_dir.mkdir(parents=True,exist_ok=True)
        jpath=person_dir/f"{safe_filename(canon)}_{year}.json"
        with open(jpath,"w",encoding="utf-8") as f:
            json.dump({k:v for k,v in blob.items() if k!="seen"},f,indent=2,ensure_ascii=False)
