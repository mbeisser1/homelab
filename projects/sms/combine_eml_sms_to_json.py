...
def canonical_name(name_or_addr: Optional[str]) -> Optional[str]:
    ...
    if key in ALIAS:
        return ALIAS[key]
    return (
        name_or_addr if sanitize_phoneish(name_or_addr) is None else ph
    ) or name_or_addr
...
def guess_direction_and_parties(
    ...
    if from_self == to_self:
        ...
        print(
            f"[WARN] Ambiguous self detection for {mid}: From={from_addr} To={to_addr}",
            file=sys.stderr,
        )
    ...
    return (
        "out",
        to_name or from_name,
        to_addr or from_addr or person_addr_hdr,
        from_addr or to_addr,
    )
...
def unique_participants(part_list: List[dict]) -> List[dict]:
    ...
    for p in part_list:
        ...
        if key not in out:
            out[key] = dict(
                name=p.get("name"), addr=p.get("addr"), phone=p.get("phone")
            )
        else:
            cur = out[key]
    ...
    return sorted(
        out.values(),
        key=lambda x: (x.get("phone") or "", x.get("addr") or "", x.get("name") or ""),
    )
...
def combine_for_person(person_key: str, messages: List[ParsedMessage], outdir: str):
    ...
    if is_group:
        label_elems = [
            canonical_name(
                p.get("name") or p.get("phone") or p.get("addr") or "unknown"
            )
            for p in dedup_parts
        ]
        subj_name = "Group: " + ", ".join(sorted(label_elems))
        labels = [
            safe_filename(p.get("phone") or p.get("addr") or p.get("name") or "unknown")
            for p in dedup_parts
        ]
        ...
        key_elems = sorted(
            (p.get("phone") or p.get("addr") or p.get("name") or "unknown")
            for p in dedup_parts
        )
        ...
        name = canonical_name(
            most_common_name or unique.get("name") or unique.get("phone") or addr
        )
        ...
        json_obj["messages"].append(
            {
                ...
                "sender": {
                    ...
                    "label": canonical_name(
                        pm.sender_name or pm.sender_phone or pm.sender_addr or "unknown"
                    ),
                },
...
def main():
    ap = argparse.ArgumentParser(
        description="Combine SMS/MMS .eml files into per-thread EML+JSON (1:1 and group)."
    )
    ...
    ap.add_argument(
        ...
        "--outdir",
        default=None,
        help="Output directory (default: scriptname_YYMMDD_HHMMSS)",
    )
    ap.add_argument(
        "--self-email",
        action="append",
    ...
    for thread_key, msgs in groups.items():
        eml_path, json_path, reason, dedup_removed = combine_for_person(
            thread_key, msgs, outdir
        )
    ...
    report = {
        ...
        "totals": {
            ...
            "messages_written": sum(n for _, _, _, n, _ in summary)
            - sum(d for *_, d in summary),
