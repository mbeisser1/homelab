...
for entry in "${DEVICES[@]}"; do
	...
	# Evaluate
	ALERTS=0
	[[ "$DEFECTS" =~ ^[0-9]+$ && "$DEFECTS" -gt 0 ]] && {
		log "ALERT: $dev has $DEFECTS grown defects"
		ALERTS=1
	}
	[[ "$NONMED" =~ ^[0-9]+$ && "$NONMED" -gt 0 ]] && {
		log "ALERT: $dev reports $NONMED non-medium errors"
		ALERTS=1
	}
	[[ "$READ_ERR" =~ ^[0-9]+$ && "$READ_ERR" -gt 0 ]] && {
		log "ALERT: $dev has $READ_ERR read errors"
		ALERTS=1
	}
	[[ "$WRITE_ERR" =~ ^[0-9]+$ && "$WRITE_ERR" -gt 0 ]] && {
		log "ALERT: $dev has $WRITE_ERR write errors"
		ALERTS=1
	}
	[[ "$VERIFY_ERR" =~ ^[0-9]+$ && "$VERIFY_ERR" -gt 0 ]] && {
		log "ALERT: $dev has $VERIFY_ERR verify errors"
		ALERTS=1
	}
