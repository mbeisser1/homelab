...
Tracked configs: [`nas-dev/etc/fstab`](../nas-dev/etc/fstab), [`nas-dev/etc/snapraid.conf`](../nas-dev/etc/snapraid.conf). On the host these live at `/etc/fstab` and `/etc/snapraid.conf`.
...
The script prints disk size, model, serial, WWN, filesystem, UUID, and mount point for every block device. Run with `sudo` for complete serial and partition-table data.
...
Excluded from parity (high-churn or non-data paths): `.cache/`, trash folders, `snapraid.content`, `*.tmp`, `/gaming/batocera/userdata/system/`, and others listed in the config.
...
| [`snapraid_sync.sh`](../nas-dev/scripts/snapraid_sync.sh) | no (disabled in crontab) | superseded by `cron_backup.sh` |
| [`snapraid_maint.sh`](../nas-dev/scripts/archive/snapraid_maint.sh) | no | archived, not scheduled |

Scrub and standalone sync scripts email `snapraid@bitrealm.dev`. The backup script emails `nas-dev@bitrealm.dev`.

## SMART health checks

### Automated report

[`nas-dev/scripts/smart_report.sh`](../nas-dev/scripts/smart_report.sh) scans SATA/SCSI drives (the array behind the LSI HBA), collects SMART attributes, and emails results to `snapraid@bitrealm.dev`.

```bash
sudo bash nas-dev/scripts/smart_report.sh
```

Suggested monthly cron (add on the host if desired):

```cron
0 4 1 * * /path/to/homelab/nas-dev/scripts/smart_report.sh >/dev/null 2>&1
```
